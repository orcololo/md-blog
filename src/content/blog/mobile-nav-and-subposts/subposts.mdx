...
<Callout variant="example">
  Consider this structure: ``` blog/ getting-started.mdx react-tutorial/
  index.mdx components.mdx state.mdx advanced-patterns.mdx ```
</Callout>

Navigation depends on context:

<Callout>
  - From `getting-started.mdx`: next goes to `react-tutorial/index.mdx` - From
  `react-tutorial/components.mdx`: next goes to `state.mdx`, previous to
  `index.mdx` - From `react-tutorial/state.mdx`: previous goes to
  `components.mdx`, parent goes to `index.mdx`
</Callout>
